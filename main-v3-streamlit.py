...
def main():
    ...
    with col1:
        ...
        file_path = st.text_input("输入文件夹路径", placeholder="例如：/path/to/your/files")
        ...
        st.markdown("## ❓ 提问")
        question = st.text_area("输入问题", placeholder="例如：本文档的主要观点是什么？", height=100)
        if st.button("🔍 开始提问", key="ask_btn"):
            if question and 'file_list' in st.session_state:
                with st.spinner("生成回答中..."):
                    answer_container = st.empty()
                    for answer in rag_chain(question):
                        answer_container.text_area("回答", value=answer, height=400)
            else:
