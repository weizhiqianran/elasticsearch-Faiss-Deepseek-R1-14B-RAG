...
def upload_files():
    ...
    ElasticsearchStore.from_documents(
        documents=[
            Document(
                page_content=chunk.page_content,
                metadata={**dict(chunk.metadata), "retriever_source": "es"}
...
def rag_chain(question):
    ...
    response = session.post(
        f"{OLLAMA_HOST}/api/generate",  # 使用配置的 OLLAMA_HOST
        json={
...
def check_environment():
    try:
        model_check = session.post(
            f"{OLLAMA_HOST}/api/show",  # 使用配置的 OLLAMA_HOST
        ...
        response = session.get(
            f"{OLLAMA_HOST}/api/tags",  # 使用配置的 OLLAMA_HOST
...
if __name__ == "__main__":
    ...
    try:
        ollama_check = session.get(OLLAMA_HOST, timeout=5)  # 使用配置的 OLLAMA_HOST
